...
<div class="space-y-6">
    <div>
        <h1 class="text-2xl font-bold text-gray-800">إضافة مشرف جديد</h1>
        <p class="text-gray-500 mt-1">إضافة حساب مشرف جديد بصلاحيات محدودة</p>
    </div>
    ...
    <form action="{{ route('admin.admins.store') }}" method="POST"
        ...
        <div>
            <label for="password" class="block text-sm font-medium text-gray-700 mb-1">كلمة المرور <span class="text-red-500">*</span></label>
            <input type="password" name="password" id="password"
                   class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gold-500 focus:border-gold-500 outline-none transition-colors text-sm" required minlength="8">
        </div>

        <div class="bg-amber-50 border border-amber-200 text-amber-700 rounded-lg p-4 text-sm">
            <i class="fas fa-info-circle ml-1"></i>
            هذا المشرف ستكون صلاحياته محدودة (لا يمكنه الوصول إلى الإعدادات العامة أو إدارة المشرفين)
        </div>
