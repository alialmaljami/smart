<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-500 text-sm mt-1">@if(isset($gallery)) تعديل بيانات الفيديو @else إضافة فيديو إلى المعرض @endif</p>
        </div>
    ...
    <form action="{{ isset($gallery) ? route($config['route_prefix'] . '.update', [$type, $gallery]) : route($config['route_prefix'] . '.store', $type) }}"
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 bg-gradient-to-l from-gray-50 to-white">
                <h2 class="font-bold text-gray-800 flex items-center gap-2">
                    <i class="fas fa-video text-gold-600"></i>
                    معلومات الفيديو
                </h2>
            </div>
            <div class="p-6 space-y-5">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <input type="text" name="slug" id="slug" value="{{ old('slug', $gallery->slug ?? '') }}"
                               class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gold-500/30 focus:border-gold-500 outline-none transition-all duration-200 text-sm" placeholder="اختياري">
                    </div>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                    <div>
                        <label for="youtube_id" class="block text-sm font-medium text-gray-700 mb-1.5">YouTube ID</label>
                        <input type="text" name="youtube_id" id="youtube_id" value="{{ old('youtube_id', $gallery->youtube_id ?? '') }}"
                               class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gold-500/30 focus:border-gold-500 outline-none transition-all duration-200 text-sm" placeholder="dQw4w9WgXcQ">
                    </div>
                    <div>
                        <label for="vimeo_id" class="block text-sm font-medium text-gray-700 mb-1.5">Vimeo ID</label>
                        <input type="text" name="vimeo_id" id="vimeo_id" value="{{ old('vimeo_id', $gallery->vimeo_id ?? '') }}"
                               class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gold-500/30 focus:border-gold-500 outline-none transition-all duration-200 text-sm" placeholder="76979871">
                    </div>
                    <div>
                        <label for="video_url" class="block text-sm font-medium text-gray-700 mb-1.5">رابط الفيديو المباشر</label>
                        <input type="url" name="video_url" id="video_url" value="{{ old('video_url', $gallery->video_url ?? '') }}"
                               class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gold-500/30 focus:border-gold-500 outline-none transition-all duration-200 text-sm" placeholder="https://example.com/video.mp4">
                    </div>
                </div>
                ...
                <div>
                    <label for="image" class="block text-sm font-medium text-gray-700 mb-1.5">صورة مصغرة</label>
                    @if(isset($gallery) && $gallery->image)
                        <div class="mb-3">
                            <img src="{{ asset('storage/' . $gallery->image) }}" class="w-48 h-28 rounded-lg object-cover border border-gray-200 shadow-sm">
                        </div>
                    @endif
            ...
            <div class="p-6 space-y-5">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                    <div>
                        ...
                        <select name="category_id" id="category_id" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gold-500/30 focus:border-gold-500 outline-none text-sm">
                    ...
                    <div>
                        ...
                        <select name="service_id" id="service_id" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gold-500/30 focus:border-gold-500 outline-none text-sm">
                    ...
                    <div>
                        ...
                        <select name="project_id" id="project_id" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gold-500/30 focus:border-gold-500 outline-none text-sm">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div class="p-6 space-y-4">
                <div>
                    ...
                    <input type="text" name="meta_title" id="meta_title" value="{{ old('meta_title', $gallery->meta_title ?? '') }}"
        ...
        <div class="flex items-center gap-3 bg-white rounded-xl shadow-sm border border-gray-200 p-5">
            ...
            <a href="{{ route($config['route_prefix'] . '.index') }}" class="inline-flex items-center gap-2 px-6 py-2.5 text-gray-600 hover:text-gray-800 bg-white border border-gray-300 rounded-lg text-sm font-medium transition-colors hover:bg-gray-50">إلغاء</a>
        </div>
    </form>
</div>
