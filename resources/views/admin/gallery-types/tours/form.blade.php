<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-500 text-sm mt-1">أضف جولة 360 لمشاريعك باستخدام Matterport, Kuula أو أي خدمة</p>
        </div>
    ...
    <form action="{{ isset($gallery) ? route($config['route_prefix'] . '.update', [$type, $gallery]) : route($config['route_prefix'] . '.store', $type) }}"
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 bg-gradient-to-l from-gray-50 to-white">
                <h2 class="font-bold text-gray-800 flex items-center gap-2"><i class="fas fa-vr-cardboard text-gold-600"></i> معلومات الجولة</h2>
            </div>
            <div class="p-6 space-y-5">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        <label for="tour_url" class="block text-sm font-medium text-gray-700 mb-1.5">رابط الجولة 360 <span class="text-red-500">*</span></label>
                        <input type="url" name="tour_url" id="tour_url" value="{{ old('tour_url', $gallery->tour_url ?? '') }}"
                               ...
                               placeholder="https://my.matterport.com/show/?m=..." required>
                        <p class="text-xs text-gray-400 mt-1">رابط التضمين من Matterport, Kuula, أو أي خدمة جولات 360</p>
                    </div>
                </div>
                <div>
                ...
                <div>
                    ...
                    @if(isset($gallery) && $gallery->image)
                        <div class="mb-3"><img src="{{ asset('storage/' . $gallery->image) }}" class="w-48 h-28 rounded-lg object-cover border border-gray-200 shadow-sm"></div>
                    @endif
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 bg-gradient-to-l from-gray-50 to-white">
                <h2 class="font-bold text-gray-800 flex items-center gap-2"><i class="fas fa-link text-gold-600"></i> الربط والتصنيف</h2>
            </div>
