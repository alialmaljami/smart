<div class="space-y-6">
    ...
    <form action="{{ isset($material) ? route('admin.materials.update', $material) : route('admin.materials.store') }}"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                <label for="tag_ids" class="block text-sm font-medium text-gray-700 mb-1">العلامات (Tags)</label>
                <select name="tag_ids[]" id="tag_ids" multiple
                    ...
                    @foreach($allTags ?? [] as $t)
                        <option value="{{ $t->id }}"
                            {{ (old('tag_ids') && in_array($t->id, old('tag_ids'))) || (isset($material) && $material->tagItems->contains($t->id)) ? 'selected' : '' }}>
                            {{ $t->name }}
                        </option>
                    @endforeach
                </select>
