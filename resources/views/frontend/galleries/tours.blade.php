...
@push('meta')
<meta name="description" content="{{ __('360 Virtual Tours') . ' - ' . __('Smart Designer Decorations') }}">
<meta property="og:title" content="{{ __('360 Virtual Tours') }} - {{ __('Smart Designer Decorations') }}">
<meta property="og:description" content="{{ __('Explore our 360 virtual tours of decoration projects') }}">
@endpush

@section('title', __('360 Virtual Tours') . ' - ' . __('Smart Designer Decorations'))
...
<section class="relative pt-20 pb-12 md:pt-40 md:pb-24 overflow-hidden bg-gradient-to-br from-[var(--navy)] via-[var(--navy)]/95 to-[var(--navy)]">
    ...
    <div class="container mx-auto px-4 relative z-10">
        <div data-aos="fade-up" class="text-center">
            ...
            <h1 class="text-2xl sm:text-3xl md:text-6xl font-black text-[var(--text-heading)] mt-3 mb-4">{{ __('360 Virtual Tours') }}</h1>
            <p class="text-[var(--text-light)] max-w-2xl mx-auto text-lg">{{ __('Explore our projects with immersive 360 virtual tours') }}</p>
        </div>
    </div>
</section>
...
<section class="py-6 bg-[var(--white)] border-b border-[var(--stone)]">
    <div class="container mx-auto px-4">
        <div class="flex flex-wrap items-center justify-center gap-3">
            ...
            <a href="{{ route('gallery.tours') }}" class="px-5 py-2 rounded-full text-sm font-medium transition-all duration-300 bg-[var(--gold)] text-white">{{ __('360 Tours') }}</a>
        ...
        @else
            <div class="text-center py-20">
                <div class="w-24 h-24 mx-auto mb-6 rounded-full bg-[var(--stone)] flex items-center justify-center">
                    <i class="fas fa-vr-cardboard text-3xl text-[var(--text-light)]"></i>
                </div>
                <h3 class="text-2xl font-bold text-[var(--gold)] mb-2">{{ __('No 360 tours yet') }}</h3>
                <p class="text-[var(--text-light)]">{{ __('Tours will be added soon') }}</p>
            </div>
        @endif
    </div>
</section>
