...
@section('title', ($service->meta_title ?? $service->name) . ' - ديكورات المصمم الذكي')
...
<section class="pt-28 pb-4 bg-[var(--cream)]">
    <div class="container mx-auto px-4">
        <nav class="flex items-center space-x-2 space-x-reverse text-sm text-[var(--text-light)]">
            ...
            <a href="{{ route('services') }}" class="hover:text-[var(--gold)] transition-colors">خدماتنا</a>
            <i class="fas fa-chevron-left text-xs"></i>
    ...
    <div class="container mx-auto px-4 relative z-10">
        <div class="grid md:grid-cols-2 gap-8 items-center">
            <div data-aos="fade-left">
                <h1 class="text-4xl md:text-5xl font-black text-white mb-4">{{ $service->name }}</h1>
        ...
        <a href="{{ route('contact') }}" class="inline-flex items-center btn-primary px-6 py-3 rounded-lg font-bold mt-6">
            <x-icon name="phone" class="w-5 h-5 inline-block ml-2 align-middle" /> اطلب الخدمة الآن
        </a>
            </div>
            @if($service->image)
                <div data-aos="fade-right" class="relative">
                    <div class="rounded-2xl overflow-hidden">
                        <img src="{{ asset('storage/' . $service->image) }}" alt="{{ $service->name }}" class="w-full h-80 object-cover">
                    </div>
    ...
    <section class="py-16 bg-[var(--cream)]">
        <div class="container mx-auto px-4">
            <div data-aos="fade-up" class="text-center mb-12">
                <h2 class="text-3xl font-black text-[var(--text-heading)]">معرض الصور</h2>
                <div class="section-divider"></div>
            </div>
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                @foreach($images as $image)
                    <div data-aos="zoom-in" data-aos-delay="{{ $loop->index * 50 }}" class="img-zoom rounded-xl overflow-hidden h-48">
                        <img src="{{ asset('storage/' . $image) }}" alt="{{ $service->name }}" class="w-full h-full object-cover">
                    </div>
                @endforeach
            </div>
        </div>
...
@if(count($videos))
    <section class="py-16 bg-[var(--white)]">
        <div class="container mx-auto px-4">
            <div data-aos="fade-up" class="text-center mb-12">
                <h2 class="text-3xl font-black text-[var(--text-heading)]">الفيديوهات</h2>
                <div class="section-divider"></div>
            </div>
...
@if($relatedProjects->count())
    <section class="py-16 bg-[var(--cream)]">
        <div class="container mx-auto px-4">
            <div data-aos="fade-up" class="text-center mb-12">
                <h2 class="text-3xl font-black text-[var(--text-heading)]">مشاريع ذات صلة</h2>
                <div class="section-divider"></div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
...
@if($relatedMaterials->count())
    <section class="py-16 bg-[var(--white)]">
        <div class="container mx-auto px-4">
            <div data-aos="fade-up" class="text-center mb-12">
                <h2 class="text-3xl font-black text-[var(--text-heading)]">مواد ديكور ذات صلة</h2>
                <div class="section-divider"></div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div data-aos="fade-up" class="card-elegant bg-[var(--white)] rounded-xl overflow-hidden">
                        ...
                        <div class="p-4">
                            ...
                            <a href="{{ route('material.category.show', $category->slug) }}" class="inline-flex items-center text-[var(--gold)] text-sm font-bold mt-2">
                                تصفح المجموعة <i class="fas fa-arrow-left mr-1"></i>
                            </a>
                        </div>
                    </div>
                @endforeach
...
<section class="py-8 bg-[var(--white)] border-t border-[var(--stone)]">
    <div class="container mx-auto px-4 text-center">
        <span class="text-[var(--text-light)] ml-2">شارك هذه الخدمة:</span>
        <div class="inline-flex space-x-2 space-x-reverse" dir="ltr">
            ...
            <a href="https://twitter.com/intent/tweet?text={{ urlencode($service->name) }}&url={{ urlencode(request()->url()) }}" target="_blank" rel="noopener noreferrer" class="w-10 h-10 rounded-full bg-[#000000] text-white flex items-center justify-center hover:scale-110 transition-transform"><x-icon name="x_twitter" class="w-5 h-5" /></a>
